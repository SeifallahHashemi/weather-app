...
const DigitButton = (props) => {
    ...
    return (
        <>
            {buttons.map(button => {
                return (
                    <button key={button.id} onClick={addClickHandler}
                            data-operation={button.operation}>{button.digit}</button>
                )
            })}
            <button className="span-two" onClick={clearButtonHandler}>AC</button>
            <button className="span-two" onClick={deleteButtonHandler}>DL</button>
        </>
    )
};
